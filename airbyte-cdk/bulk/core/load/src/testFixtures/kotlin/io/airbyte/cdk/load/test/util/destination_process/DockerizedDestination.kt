...
class DockerizedDestination(
        ...
        destinationDataChannels =
            when (dataChannelMedium) {
                DataChannelMedium.STDIO ->
                    arrayOf(BufferedWriter(OutputStreamWriter(process.outputStream, Charsets.UTF_8)))
                DataChannelMedium.SOCKETS -> {
                    (0 until NUM_SOCKETS).map {
                        val sidecarPort = TcpPortReserver.findAvailablePort()
                        sidecarProcess = startSidecar(sidecarPort, makeSocketPath(it))
                        val tcpSocketWriter =
    ...
    private fun startSidecar(sidecarPort: Int, socketPath: String): Process {
                            ...
                            socketPaths.forEach {
                                if (
                                    message.log.message.contains(
                                        "Socket $it connected for reading"
    ...
    override suspend fun sendMessage(message: AirbyteMessage, broadcast: Boolean) {
        ...
        val dataChannels = if (broadcast) {
            destinationDataChannels
        } else {
            arrayOf(destinationDataChannels[dataChannelIndex.rotate(destinationDataChannels.size)])
        }
